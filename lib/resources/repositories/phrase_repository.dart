...
class PhraseRepository {
  ...
  Future<void> create(PhraseRow phrase, BookRow book) async {
    ...
    _db.runTransaction((transaction) async {
      DocumentReference _bookDoc = await _db.collection('books').doc(book.bookId);
      transaction.set(_bookDoc, book.toMap());

      DocumentReference _phraseDoc = await _db.collection('phrases').doc(phrase.phraseId);
  ...
  Future<List<Map<String, dynamic>>> list(String userId) async {
    ...
    QuerySnapshot _phraseQs = await _db.collection('phrases')
        .where('postedUserId', isEqualTo: userId)
        .get();
    List<PhraseRow> phrases = _phraseQs.docs.map((doc) => PhraseRow.fromSnapshot(doc)).toList();
    ...
    await Future.forEach(chunkedBookIds, (bookIds) async {
      QuerySnapshot _bookQs = await _db.collection('books')
          .where(FieldPath.documentId, whereIn: bookIds)
          .get();
      books.addAll(_bookQs.docs.map((doc) => BookRow.fromSnapshot(doc)).toList());
    });


    return phrases.map((phrase) => {'phrase': phrase, 'book': books.singleWhere((b) => b.bookId == phrase.bookId)}).toList();
  }
  ...
  Future<List<PhraseRow>> listByUserIds(List<String> userIds) async {
    final snapshot = await FirebaseFirestore.instance
        ...
        .get();
    return snapshot.docs.map((doc) => PhraseRow.fromSnapshot(doc)).toList(growable: false);
  }
}